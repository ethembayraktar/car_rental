...
class MyIyzico:
    ...
    def send_non_threed_request(self, iyzico_request):
        ...
        print(iyzico_request)
        print(headers)
        payment_response = requests.post(url=nonThreedUrl, data=iyzico_request, headers=headers)
        # payment_json = json.loads(payment_response.read().decode('utf-8'))
